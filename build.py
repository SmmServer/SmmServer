...
def build_windows():
    ...
    res = run_command(["flet", "build", "windows", "--yes", "--no-rich-output"])
    if res == 0:
...
def build_linux():
    ...
    return run_command(["flet", "build", "linux", "--yes", "--no-rich-output"])

def build_android():
    print("Building for Android (APK)...")
    res = run_command(["flet", "build", "apk", "--yes", "--no-rich-output"])
    if res == 0:
